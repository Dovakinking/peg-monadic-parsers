...
-- | A PEG grammar.  The start rule is interpreted with an implicit end-of-input
-- check by PEG.Matcher, so membership means that the whole input was consumed.
...
-- | Parsing expressions supported by the project.
--
-- The language intentionally excludes semantic actions: the task is membership
-- checking, so each parser returns only ().
data Expr
  = Epsilon
  | Literal String
  | AnyChar
  | CharClass Bool [ClassAtom]  -- ^ Bool=True means negated class [^...].
  | NonTerminal Name
  | Sequence [Expr]
  | Choice [Expr]
  | And Expr
