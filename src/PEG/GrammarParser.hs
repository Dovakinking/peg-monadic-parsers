...
-- | Parser for the grammar-description language.
...
-- PEG priority used here:
-- choice / has the lowest priority,
-- sequence is juxtaposition,
-- prefixes & and !,
-- suffixes *, +, ?,
-- primary expressions.
expressionP :: P Expr
...
nonTerminalP :: P Expr
nonTerminalP = do
  -- Peggy-style grammars can separate rules only by whitespace.  Therefore an
  -- identifier followed by '=' at the current top-level position starts the next
  -- rule and must not be swallowed as a nonterminal reference.
  notRuleStart
...
classAtomP = try rangeP <|> (Single <$> classCharP)
  where
    rangeP = do
      a <- classCharP
      void (char '-')
      -- A dash before the closing bracket is a literal dash, not a range marker.
      _ <- lookAhead (noneOf "]")
      b <- classCharP
      pure (Range a b)
...
-- Small local helpers keep the import list explicit and avoid relying on
-- newer Prelude exports.
