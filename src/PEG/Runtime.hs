...
-- | Memoized parser state.  'position' is an offset in the original input.
...
data MemoEntry
  = InProgress
  | Done Bool Int        -- ^ success flag and ending position for successful rules
  deriving (Eq, Show)

-- | A deterministic PEG parser.  The result is Nothing on failure and Just a on
-- success.  Failure rolls the position back to the point where the composite
-- parser started, while memo-table effects are preserved.
...
-- | Run a parser without automatically requiring end-of-input.
...
-- | Membership check: parser must succeed and consume the whole input.
...
-- | Memoize a named rule.  This is the packrat part of the implementation.
